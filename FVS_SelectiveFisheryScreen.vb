...
Public Class FVS_SelectiveFisheryScreen
    ...
    Private Sub btn_MSFreport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_MSFreport.Click
        ...
        'DD added in the coho MSF report on 5.27.2021


        'Step 1 for both Chinook and Coho: Open and access Microsoft Excel
        ...
        appXL.Visible = True
        '************************************
        appXL.Application.Interactive = False
        ' Add a new workbook.
        ...
        ' AutoFit columns A:D.
        raXL = shXL.Range("A1", "M2000")
        raXL.NumberFormat = "#,##0"

        If SpeciesName = "COHO" Then
            'Step 2: Get a list of fisheries and time steps for which MSF regulations exist
            CountMSF = 0
            For Fish = 1 To NumFish
                For TStep = 1 To NumSteps
                    If FisheryFlag(Fish, TStep) = 7 Or FisheryFlag(Fish, TStep) = 17 Or FisheryFlag(Fish, TStep) = 27 Or FisheryFlag(Fish, TStep) = 28 Or FisheryFlag(Fish, TStep) = 8 Or FisheryFlag(Fish, TStep) = 18 Then
                        'Only do WA Marine Sport Fisheries with MSF regs
                        'If Fish = 33 Or Fish = 37 Or Fish = 40 Or Fish = 41 Or Fish = 45 Or Fish = 48 Or Fish = 91 Or Fish = 92 Or Fish = 93 Or Fish = 106 Or Fish = 107 Or Fish = 115 Or Fish = 118 Or Fish = 129 Or Fish = 136 Or Fish = 152 Then
                        If MSFFisheryQuota(Fish, TStep) > 0 Or MSFFisheryScaler(Fish, TStep) > 0 Then
                            MSFList(Fish, TStep) = 1
                            CountMSF = CountMSF + 1
                            If TStep = 1 Then
                                StepName(Fish, TStep) = "Jan-June"
                            ElseIf TStep = 2 Then
                                StepName(Fish, TStep) = "July"
                            ElseIf TStep = 3 Then
                                StepName(Fish, TStep) = "August"
                            ElseIf TStep = 4 Then
                                StepName(Fish, TStep) = "September"
                            Else
                                StepName(Fish, TStep) = "Oct-December"
                            End If
                        End If
                        'End If
                    Else
            ...
            Header = "Table 1. FRAM Estimates of coho encounters and mortalities in WA sport MSFs (Model Run: " & _
            ...
            For Fish = 1 To NumFish
                For TStep = 1 To NumSteps
                    If MSFList(Fish, TStep) = 1 Then
                        ...
                        CellAdd = "C" & R + 3
                        If ShakerMortRate(Fish, TStep) > 0 Then ' Avoid divide by zeros here
                            shXL.Range(CellAdd).Value = TotMSubWDFW(Fish, TStep) / ShakerMortRate(Fish, TStep)
                        Else 'if there are no shakers as seems to the be the case for coho
            ...
            ' of Excel's lifetime.
            appXL.Application.Interactive = True
            appXL.Visible = True
            ...
            For Fish = 1 To NumFish
                For TStep = 2 To NumSteps
                    If FisheryFlag(Fish, TStep) = 7 Or FisheryFlag(Fish, TStep) = 17 Or FisheryFlag(Fish, TStep) = 27 Or FisheryFlag(Fish, TStep) = 28 Or FisheryFlag(Fish, TStep) = 8 Or FisheryFlag(Fish, TStep) = 18 Then
                        ...
                        If Fish = 18 Or Fish = 22 Or Fish = 27 Or Fish = 36 Or Fish = 42 Or Fish = 45 Or Fish = 48 Or Fish = 53 Or Fish = 54 Or Fish = 56 Or Fish = 57 Or Fish = 60 Or Fish = 62 Or Fish = 64 Or Fish = 67 Then
                            If MSFFisheryQuota(Fish, TStep) > 0 Or MSFFisheryScaler(Fish, TStep) > 0 Then
                                MSFList(Fish, TStep) = 1
                                CountMSF = CountMSF + 1
                                If TStep = 2 Then
